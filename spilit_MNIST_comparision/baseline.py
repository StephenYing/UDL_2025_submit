...
def train_mfvi_epoch(model, loader, optimizer, criterion,kl_weight, device, task_id=0, num_samples=1):
    model.train()
...
def run_vcl_pytorch(data_gen, input_dim, hidden_sizes, output_dim, num_tasks,
    ...
    for task_id in range(num_tasks):
        ...
        if task_id == 0:
            ...
            for ep in range(num_epochs_vanilla):
                train_vanilla_epoch(vanilla_model, loader, optimizer, criterion,
                                    device, head_idx)

            current_mfvi_model = MFVI_NN(input_dim, hidden_sizes, output_dim,
                                         num_heads).to(device)
            ...
            if combined_x.shape[0] > 0:
                ...
                for ep in range(num_epochs_mfvi):
                    train_mfvi_epoch(current_mfvi_model, loader, optimizer,
                                     criterion, kl_weight, device,
                                     head_idx, train_mc_samples)

                current_mfvi_model.set_prior_from_posterior()

        print(" Evaluating ...")
        accs = get_scores_pytorch(current_mfvi_model, x_testsets, y_testsets,
                                  device, single_head, num_tasks)
    ...
    print(f"[VCL] Finished all tasks in {total_time:.1f}s, final avg acc={final_acc:.4f}")
...
# laplace VCL
def train_mfvi_epoch_laplace(model, loader, optimizer, criterion,
                             kl_weight, device, task_id=0,
                             num_samples=1):
    model.train()
    total_loss = 0.0
    total_nll = 0.0
...
def run_vcl_pytorch_laplace(data_gen,input_dim, hidden_sizes,output_dim,num_tasks,num_epochs_vanilla,num_epochs_mfvi, batch_size,lr,
    ...
    for task_id in range(num_tasks):
        ...
        if task_id==0:
            print("  Training VanillaNN for first task ...")
            ...
            current_mfvi_model = MFVI_NN_AnalyticLaplace(
                input_dim, hidden_sizes, output_dim,
                num_heads, prior_b=prior_b
            ).to(device)
            ...
            if combined_x.shape[0] > 0:
                ...
                for ep in range(num_epochs_mfvi):
                    train_mfvi_epoch_laplace(
                        current_mfvi_model, loader, optimizer, criterion,
                        kl_weight, device, head_idx,
                        num_samples=train_num_mc
                    )
