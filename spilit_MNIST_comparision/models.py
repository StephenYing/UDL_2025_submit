...
def kl_gaussian_laplace_allparams(weight_mean, weight_log_var, laplace_mean_w, laplace_log_b_w,bias_mean, bias_log_var,laplace_mean_b, laplace_log_b_b):
...
class MFVI_Layer(nn.Module):
    ...
    def kl_divergence(self):
        kl_w = 0.5 * torch.sum(self.prior_weight_log_var - self.weight_log_var +(torch.exp(self.weight_log_var) +(self.weight_mean - self.prior_weight_mean) ** 2)/ torch.exp(self.prior_weight_log_var) - 1.0)
        kl_b = 0.5 * torch.sum(self.prior_bias_log_var - self.bias_log_var +(torch.exp(self.bias_log_var) +(self.bias_mean - self.prior_bias_mean) ** 2) / torch.exp(self.prior_bias_log_var) - 1.0)
        return kl_w + kl_b
...
class MFVI_NN(nn.Module):
    def __init__(self, input_size, hidden_sizes, output_size, num_tasks=1,
        ...
        in_dim = input_size
        for h_dim in hidden_sizes:
            self.layers.append(MFVI_Layer(in_dim, h_dim,
                                          initial_prior_mean,
                                          initial_prior_log_var))
            in_dim = h_dim
        if self.num_tasks == 1:
            self.output_layer = MFVI_Layer(in_dim, output_size,
                                           initial_prior_mean,
                                           initial_prior_log_var)
        else:
            self.output_layers = nn.ModuleList([
                MFVI_Layer(in_dim, output_size,
                           initial_prior_mean,
                           initial_prior_log_var)
                for _ in range(num_tasks)
            ])
